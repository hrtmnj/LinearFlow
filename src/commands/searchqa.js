...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('searchqa')
    .setDescription('Search for a QA ticket by ID')
  ...
  async execute(interaction) {
    ...
    try {
      ...
      // Check if issue has QA label by NAME instead of ID
      const labels = await issue.labels();
      const hasQALabel = labels.nodes.some(label => label.name.toUpperCase() === 'QA');
      ...
      if (!hasQALabel) {
        const errorEmbed = new EmbedBuilder()
          ...
          .setDescription(
            wasMoved 
              ? `Ticket **${ticketId}** has been moved to **${currentIdentifier}** and no longer has the QA label. You can only search QA tickets.`
              : `Ticket **${ticketId}** does not have the QA label. You can only search QA tickets.`
          )
          .setTimestamp();
